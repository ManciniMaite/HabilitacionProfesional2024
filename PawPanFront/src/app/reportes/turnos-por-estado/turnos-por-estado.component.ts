...
@Component({
  ...
  templateUrl: './turnos-por-estado.component.html',
  styleUrl: './turnos-por-estado.component.scss'
})
export class TurnosPorEstadoComponent {
  
  displayedColumns: string[] = ['estado','cantidad', 'porcentaje']
  dataSource:PorcentajesTurnosPorEstado[]=[];
  ...
  getDatos(){
    ...
    this.service.getTurnosPorEstados(rq).subscribe({
      next:(data)=>{
  ...
  getTotalTurnos(): number {
    ...
    if(this.dataSource.length>0){
      this.dataSource.forEach(item => total+=item.cantidad)
    }
    return total
  }
}
